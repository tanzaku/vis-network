...
class LayoutEngine {
   ...
   * @private
   */
  _placeNodesByHierarchy3(distribution) {
    ...
    let calcCrossing2 = (i, j) => {
      return (calcCrossing1(i) + calcCrossing1(j)) * 1;
    };
    ...
    for (let i = 0; i < edges.length; i++) {
      for (let j = 0; j < i; j++) {
        ...
        crossableEdgePairs[edges[j].from.id].push([i, j]);
        // crossableEdgePairs[edges[j].to.id].push([i, j]);
      }
    }
  ...
  _placeNodesByHierarchy(distribution) {
    this._placeNodesByHierarchy3(distribution);
    return;
