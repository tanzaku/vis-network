...
class LayoutEngine {
  ...
  _placeNodesByHierarchy3(distribution) {
    ...
    let calcCrossing1 = (i) => {
      let res = 0;
      for (let j of crossableEdgePairs[i]) {
        const e1 = edges[i];
        const e2 = edges[j];
    ...
    let calcCrossing2 = (i, j) => {
      return calcCrossing1(i) + calcCrossing1(j);
    };
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      for (let j = 0; j < i; j++) {
        ...
        if (Math.max(y00, y01) <= Math.min(y10, y11) || Math.max(y10, y11) <= Math.min(y00, y01)) {
          continue;
        }
        ...
        crossableEdgePairs[i].push(j);
        crossableEdgePairs[j].push(i);
      }
    }
