...
class LayoutEngine {
  ...
  _placeNodesByHierarchy(distribution) {
    ...
    // start placing all the level 0 nodes first. Then recursively position
    // their branches.
    let nodeLevel = [];
    let edges = [];
    let crossableEdgePairs = [];

    // 交差しうるペアだけ検証することで高速化できる
    // let calcCrossing = () => {
    //   let res = 0;
    //   for (let edgePair of crossableEdgePairs) {
    //     const e1 = edgePair[0];
    //     const e2 = edgePair[1];

    //     const x00 = this.direction.getPosition(e1.from);
    //     const y00 = nodeLevel[e1.from.id];
    //     const x01 = this.direction.getPosition(e1.to);
    //     const y01 = nodeLevel[e1.to.id];

    //     const x10 = this.direction.getPosition(e2.from);
    //     const y10 = nodeLevel[e2.from.id];
    //     const x11 = this.direction.getPosition(e2.to);
    //     const y11 = nodeLevel[e2.to.id];

    //     // https://qiita.com/ykob/items/ab7f30c43a0ed52d16f2
    //     const ta = (x10 - x11) * (y00 - y10) + (y10 - y11) * (x10 - x00);
    //     const tb = (x10 - x11) * (y01 - y10) + (y10 - y11) * (x10 - x01);
    //     const tc = (x00 - x01) * (y10 - y00) + (y00 - y01) * (x00 - x10);
    //     const td = (x00 - x01) * (y11 - y00) + (y00 - y01) * (x00 - x11);
    ...
    //   // for (let i = 0; i < edges.length; i++) {
    //   //   for (let j = 0; j < i; j++) {
    //   //     let x00 = this.direction.getPosition(edges[i].from);
    //   //     let y00 = nodeLevel[edges[i].from.id];
    //   //     let x01 = this.direction.getPosition(edges[i].to);
    //   //     let y01 = nodeLevel[edges[i].to.id];

    //   //     let x10 = this.direction.getPosition(edges[j].from);
    //   //     let y10 = nodeLevel[edges[j].from.id];
    //   //     let x11 = this.direction.getPosition(edges[j].to);
    //   //     let y11 = nodeLevel[edges[j].to.id];

    //   //     // https://qiita.com/ykob/items/ab7f30c43a0ed52d16f2
    //   //     var ta = (x10 - x11) * (y00 - y10) + (y10 - y11) * (x10 - x00);
    //   //     var tb = (x10 - x11) * (y01 - y10) + (y10 - y11) * (x10 - x01);
    //   //     var tc = (x00 - x01) * (y10 - y00) + (y00 - y01) * (x00 - x10);
    //   //     var td = (x00 - x01) * (y11 - y00) + (y00 - y01) * (x00 - x11);
    ...
    let calcCrossing1 = (i) => {
      let res = 0;
      for (let edgePair of crossableEdgePairs[i]) {
        const e1 = edges[edgePair[0]];
        const e2 = edges[edgePair[1]];

        res += isCrossing(e1, e2);
      }
      return res;
    };

    let calcCrossing2 = (i, j) => {
      // let common = 0;
      let res = 0;
      for (let edgePair of crossableEdgePairs[i]) {
        const e1 = edges[edgePair[0]];
        const e2 = edges[edgePair[1]];
        if (e2.from.id !== j && e2.to.id !== j) {
          res += isCrossing(e1, e2);
        // } else {
        //   common += isCrossing(e1, e2);
        }
      }

      for (let edgePair of crossableEdgePairs[j]) {
        const e1 = edges[edgePair[0]];
        const e2 = edges[edgePair[1]];
        res += isCrossing(e1, e2);
      }
      ...
      if (distribution.hasOwnProperty(level)) {
        ...
        for (let i = 0; i < nodeArray.length; i++) {
          ...
          if (this.positionedNodes[node.id] === undefined) {
            ...
            crossableEdgePairs[node.id] = [];
            for (let j = 0; j < nodeArray[i].edges.length; j++) {
              // if (nodeArray[i].edges[j].from.id === 0 || nodeArray[i].edges[j].to.id === 0) {
              if (nodeArray[i].edges[j].from.id === node.id) {
                edges.push(nodeArray[i].edges[j]);
                // console.log("CHECK", node.id, level, nodeArray[i].edges[j].from.id, nodeArray[i].edges[j].to.id);
              }
            }
          }
        }
    ...
    for (let i = 0; i < edges.length; i++) {
      // if (edges[i].from.id === 0 || edges[i].to.id === 0) {
      //   console.log("CHECK", edges[i].from.id, edges[i].to.id);
      // }
      for (let j = 0; j < i; j++) {
        let y00 = nodeLevel[edges[i].from.id];
        let y01 = nodeLevel[edges[i].to.id];
        ...
        if (Math.max(y00, y01) <= Math.min(y10, y11) + 1e-9 || Math.max(y10, y11) <= Math.min(y00, y01) + 1e-9) {
          // console.log("NG1", y00, y01, y10, y11, edges[i].from.id, edges[i].to.id, edges[j].from.id, edges[j].to.id);
          continue;
        }

        if (edges[i].from === edges[j].from || edges[i].to === edges[j].from || edges[i].from === edges[j].to || edges[i].to === edges[j].to) {
          // console.log("NG2", y00, y01, y10, y11, edges[i].from.id, edges[i].to.id, edges[j].from.id, edges[j].to.id);
          continue;
        }

        if (edges[i].to === edges[i].from || edges[j].from === edges[j].to) {
          // console.log("NG3", y00, y01, y10, y11, edges[i].from.id, edges[i].to.id, edges[j].from.id, edges[j].to.id);
          continue;
        }

        // console.log(y00, y01, y10, y11, edges[i].from.id, edges[i].to.id, edges[j].from.id, edges[j].to.id);

        // TODO from->toだけ考えて、下から決めていくのがいいかも
        crossableEdgePairs[edges[i].from.id].push([i, j]);
        crossableEdgePairs[edges[i].to.id].push([i, j]);
        crossableEdgePairs[edges[j].from.id].push([j, i]);
        crossableEdgePairs[edges[j].to.id].push([j, i]);
      }
    }

    // let numCrossing = calcCrossing();
    let distributionKeys = [];
    let nodeLevelKeys = Object.keys(nodeLevel);
    for (let level in distribution) {
      distributionKeys.push(Object.keys(distribution[level]));
    }

    // npm install C:\work\src\vis-network
    // npm run dev
    let allCrossings = () => {
      let numCrossing = 0;
      for (let i = 0; i < crossableEdgePairs.length; i++) {
        numCrossing += calcCrossing1(i);
      }
      return numCrossing;
    };

    const startTime = performance.now();
    const MAX_ITER = 50000 / 5;
    const T0 = 10.0;
    let prevCrossing = allCrossings();
    let decreaseCrossing = 0;

    // let numCrossings = [];
    // for (let i = 0; i < crossableEdgePairs.length; i++ ) {
    //   numCrossings[i] = calcCrossing1(i);
    // }

    for (let iter = 0; iter < MAX_ITER; iter++) {
      // - insert遷移を追加
      // - 下に影響するので、徐々に下から上にやっていく？
      let level = nodeLevel[nodeLevelKeys[Math.floor(Math.random() * nodeLevelKeys.length)]];
      let dk = distributionKeys[level];
      let node1 = distribution[level][dk[Math.floor(Math.random() * dk.length)]];
      let node2 = distribution[level][dk[Math.floor(Math.random() * dk.length)]];

      if (node1.id === node2.id) {
        continue;
      }

      // const prevCrossing = numCrossings[node1.id] + numCrossings[node2.id];
      const prevCrossing = calcCrossing2(node1.id, node2.id);
      // const prevCrossing = allCrossings();

      let x1 = this.direction.getPosition(node1);
      let x2 = this.direction.getPosition(node2);
      this.direction.setPosition(node1, x2, level);
      this.direction.setPosition(node2, x1, level);

      // const newCrossingNode1 = calcCrossing1(node1.id);
      // const newCrossingNode2 = calcCrossing1(node2.id);
      // const newCrossingNode12 = calcCrossing2(node1.id, node2.id);
      // const newCrossing = newCrossingNode1 + newCrossingNode2 - newCrossingNode12;
      const newCrossing = calcCrossing2(node1.id, node2.id);
      // const newCrossing = allCrossings();

      if (newCrossing - prevCrossing <= 0) {
      // if (newCrossing - prevCrossing <= -(T0 * (1.0-iter/MAX_ITER) * Math.log(Math.random()))) {
        // if (numCrossing !== newCrossing) {
        //   console.log(iter, numCrossing, newCrossing);
        // }
        // numCrossing = newCrossing;
        // numCrossings[node1.id] = newCrossingNode1;
        // numCrossings[node2.id] = newCrossingNode2;
        decreaseCrossing += newCrossing - prevCrossing;
        continue;
      }
      this.direction.setPosition(node1, x1, level);
      this.direction.setPosition(node2, x2, level);
    }
    const endTime = performance.now();
    let afterCrossing = allCrossings();
    console.log("number of nodes", crossableEdgePairs.length, "number of edges", edges.length)
    console.log("time", endTime - startTime, "[ms]", prevCrossing, decreaseCrossing, afterCrossing);
  }
